...
> ## 一句话结论（抗阶段遮蔽的类级口径）
>
> 8 个真实 jar、合计 **2252** 个顶层类：**类级干净率 96.1%（2163/2252 干净，89 个缺陷类）**，
...
> fastjson2 97.2%、guava 96.4%、spring-core 94.0%**。自托管算法往返 **5/5 逐字节一致**。三方横评（§7）里 JavaJive 的
> **类级干净率 96.1% 位列第一**（Vineflower 90.8%、CFR 79.7%），缺陷类总数 **89 vs CFR 457（少 81%） / Vineflower 208（少 57%）**。
...
| **反编译正确性** | 类级干净率（主口径） | **96.1%**（2163/2252，8 个真实 jar） | 表 A | ✅ |
...
| **核心目标库** | gson / fastjson2 / guava / spring-core 干净率 | **100% / 97.2% / 96.4% / 94.0%** | 表 A | gson GA |
| **横向对比** | 类级干净率三方排名 | **第一**（JavaJive 96.1% > Vineflower 90.8% > CFR 79.7%） | 表 E | ✅ |
...
按**抗阶段遮蔽的「类级干净率」口径**：JavaJive 在 8 个真实 jar、2252 个顶层类上的**类级干净率为 96.1%
（2163/2252，89 缺陷类），全集 0 语法错**——数字诚实、无遮蔽。**commons-codec 与 gson 达成完整往返**
...
| **合计** | | **89/2252（96.1%）** | 457/2254（79.7%） | 208/2252（90.8%） |
...
- **类级干净率第一**：JavaJive **96.1%** > Vineflower **90.8%** > CFR **79.7%**。
- **对 CFR：全面领先。** **8/8 个 jar** 上 JavaJive 缺陷类**均少于** CFR；合计缺陷类 **89 vs 457——比 CFR 少 81%**
  （干净率领先约 16 个百分点）。JSON / 序列化 / 编码族尤为悬殊：gson **0 vs 24**、fastjson2 **15 vs 90**、
  guava **20 vs 154**、commons-codec **0 vs 10**、spring-core **39 vs 117**。
- **对 Vineflower：总量领先、各擅胜场。** 合计缺陷类 **89 vs 208——比 Vineflower 少 57%**，逐 jar 各赢一部分：
