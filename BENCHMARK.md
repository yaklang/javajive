...
> ## 一句话结论（抗阶段遮蔽的类级口径）
>
> 8 个真实 jar、合计 **2252** 个顶层类：**类级干净率 93.6%（2107/2252 干净，145 个缺陷类）**，
...
> fastjson2 97.2%、guava 96.1%**。自托管算法往返 **5/5 逐字节一致**。三方横评（§7）里 JavaJive 的
> **类级干净率 93.6% 位列第一**（Vineflower 90.8%、CFR 79.7%），缺陷类总数 **145 vs CFR 457（少 68%） / Vineflower 208（少 30%）**。
...
| **反编译正确性** | 类级干净率（主口径） | **93.6%**（2107/2252，8 个真实 jar） | 表 A | ✅ |
...
| **核心目标库** | gson / fastjson2 / guava 干净率 | **100% / 97.2% / 96.1%** | 表 A | gson GA |
| **横向对比** | 类级干净率三方排名 | **第一**（JavaJive 93.6% > Vineflower 90.8% > CFR 79.7%） | 表 E | ✅ |
...
- **已达 GA、可直接投产**：**commons-codec、gson**——整树零错、重打包后外部 JVM 逐类字节码校验全通过，
  ...
  达到“拿去就能用”的工业标准。
- **高准确度、可用于分析与交叉验证**：**fastjson2 97.2%、guava 96.1%、jsoup 98.0%、snakeyaml 97.5%、
  commons-lang3 88.9%、spring-core 87.4%**——单类级别可读、可重编译比例高，适合逆向分析、漏洞审计、补丁验证等
  **以类为单位**的工程场景；整包完整往返仍有泛型擦除造型等长尾在收敛（见 §4）。
- **诚实边界**：并非所有库都已 100% 干净往返；我们**以 syntax=0 硬断言杜绝“用语法错遮蔽类型错”的虚高**，
  报告的 93.6% 是**无遮蔽的诚实值**，不是乐观估计。
...
| commons-lang3 | 345 | 176/198 | 88.9% | 22 |
| jsoup | 238 | 50/51 | 98.0% | 1 |
| snakeyaml | 231 | 119/122 | 97.5% | 3 |
| spring-core | 978 | 567/649 | 87.4% | 82 |
| fastjson2 | 681 | 514/529 | 97.2% | 15 |
| guava | 1892 | 536/558 | 96.1% | 22 |
| **合计** | | **2107/2252** | **93.6%** | **145** |

> **核心目标库**：**gson 100%（完整往返）**、commons-codec 100%（完整往返）、fastjson2 97.2%、guava 96.1%。
...
| commons-lang3 | 75 | 0 | 14/14 | no |
| jsoup | 1 | 0 | 17/17 | no |
| snakeyaml | 12 | 0 | 0/0 | no |
| spring-core | 739 | 0 | 0/0 | no |
| fastjson2 | 33 | 0 | 0/0 | no |
| guava | 34 | 0 | 0/0 | no |
...
按**抗阶段遮蔽的「类级干净率」口径**：JavaJive 在 8 个真实 jar、2252 个顶层类上的**类级干净率为 93.6%
（2107/2252，145 缺陷类），全集 0 语法错**——数字诚实、无遮蔽。**commons-codec 与 gson 达成完整往返**
...
| commons-lang3 | 345 | 22/198 (88.9%) | 46/198 (76.8%) | **6/198 (97.0%)** |
| jsoup | 238 | **1/51 (98.0%)** | 5/51 (90.2%) | 2/51 (96.1%) |
| snakeyaml | 231 | 3/122 (97.5%) | 11/123 (91.1%) | **2/121 (98.3%)** |
| spring-core | 978 | 82/649 (87.4%) | 117/649 (82.0%) | **74/649 (88.6%)** |
| fastjson2 | 681 | **15/529 (97.2%)** | 90/530 (83.0%) | 40/529 (92.4%) |
| guava | 1892 | **22/558 (96.1%)** | 154/558 (72.4%) | 66/558 (88.2%) |
| **合计** | | **145/2252（93.6%）** | 457/2254（79.7%） | 208/2252（90.8%） |
...
- **类级干净率第一**：JavaJive **93.6%** > Vineflower **90.8%** > CFR **79.7%**。
- **对 CFR：全面领先。** **8/8 个 jar** 上 JavaJive 缺陷类**均少于** CFR；合计缺陷类 **145 vs 457——比 CFR 少 68%**
  （干净率领先约 14 个百分点）。JSON / 序列化 / 编码族尤为悬殊：gson **0 vs 24**、fastjson2 **15 vs 90**、
  guava **22 vs 154**、commons-codec **0 vs 10**。
- **对 Vineflower：总量领先、各擅胜场。** 合计缺陷类 **145 vs 208——比 Vineflower 少 30%**，逐 jar 各赢一部分：
  JavaJive 赢 5 个 jar（codec **0 vs 2**、gson **0 vs 16**、jsoup **1 vs 2**、fastjson2 **15 vs 40**、guava **22 vs 66**），
  Vineflower 赢 3 个（commons-lang3 **6 vs 22**、snakeyaml **2 vs 3**、spring-core **74 vs 82**）。
...
| gson | 0 | 103 | 22 |
| commons-lang3 | 75 | 287 | 10 |
| jsoup | 1 | 35 | 3 |
| snakeyaml | 12 | 33 | 2 |
| spring-core | 739 | 764 | 614 |
| fastjson2 | 33 | 614 | 286 |
| guava | 34 | 865 | 132 |

> 行数口径同样**不作主口径**：它既被语法错遮蔽、又随内联/摊平的文件规模波动。此处仅示三方量级——JavaJive 的错误行数在
> JSON/序列化族显著低于二者（fastjson2 33 vs 614 vs 286；guava 34 vs 865 vs 132）。
