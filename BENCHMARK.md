...
> fastjson2 97.2%、guava 96.4%、spring-core 95.4%**。自托管算法往返 **5/5 逐字节一致**。三方横评（§7）里 JavaJive 的
...
- **高准确度、可用于分析与交叉验证**：**fastjson2 97.2%、guava 96.4%、jsoup 98.0%、snakeyaml 98.4%、
  spring-core 95.4%、commons-lang3 94.4%**——单类级别可读、可重编译比例高，适合逆向分析、漏洞审计、补丁验证等
...
（2173/2252，79 缺陷类），全集 0 语法错**——数字诚实、无遮蔽。**commons-codec 与 gson 达成完整往返**
...
- **对 CFR：全面领先。** **8/8 个 jar** 上 JavaJive 缺陷类**均少于** CFR；合计缺陷类 **79 vs 457——比 CFR 少 83%**
  （干净率领先约 17 个百分点）。JSON / 序列化 / 编码族尤为悬殊：gson **0 vs 24**、fastjson2 **15 vs 90**、
  guava **20 vs 154**、commons-codec **0 vs 10**、spring-core **30 vs 117**。
- **对 Vineflower：总量领先、各擅胜场。** 合计缺陷类 **79 vs 208——比 Vineflower 少 62%**，逐 jar 各赢一部分：
  JavaJive 赢 6 个 jar（codec **0 vs 2**、gson **0 vs 16**、jsoup **1 vs 2**、fastjson2 **15 vs 40**、guava **20 vs 66**、
  spring-core **30 vs 74**），snakeyaml 打平（**2 vs 2**，干净率 98.4% 微胜 98.3%），
