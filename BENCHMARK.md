...
> ## 一句话结论（抗阶段遮蔽的类级口径）
>
> 8 个真实 jar、合计 **2252** 个顶层类：**类级干净率 96.2%（2167/2252 干净，85 个缺陷类）**，
...
> fastjson2 97.2%、guava 96.4%、spring-core 94.5%**。自托管算法往返 **5/5 逐字节一致**。三方横评（§7）里 JavaJive 的
> **类级干净率 96.2% 位列第一**（Vineflower 90.8%、CFR 79.7%），缺陷类总数 **85 vs CFR 457（少 81%） / Vineflower 208（少 59%）**。
...
| **反编译正确性** | 类级干净率（主口径） | **96.2%**（2167/2252，8 个真实 jar） | 表 A | ✅ |
...
| **核心目标库** | gson / fastjson2 / guava / spring-core 干净率 | **100% / 97.2% / 96.4% / 94.5%** | 表 A | gson GA |
| **横向对比** | 类级干净率三方排名 | **第一**（JavaJive 96.2% > Vineflower 90.8% > CFR 79.7%） | 表 E | ✅ |
...
- **高准确度、可用于分析与交叉验证**：**fastjson2 97.2%、guava 96.4%、jsoup 98.0%、snakeyaml 98.4%、
  spring-core 94.5%、commons-lang3 94.4%**——单类级别可读、可重编译比例高，适合逆向分析、漏洞审计、补丁验证等
  **以类为单位**的工程场景；整包完整往返仍有泛型擦除造型等长尾在收敛（见 §4）。
- **诚实边界**：并非所有库都已 100% 干净往返；我们**以 syntax=0 硬断言杜绝“用语法错遮蔽类型错”的虚高**，
  报告的 96.2% 是**无遮蔽的诚实值**，不是乐观估计。
...
按**抗阶段遮蔽的「类级干净率」口径**：JavaJive 在 8 个真实 jar、2252 个顶层类上的**类级干净率为 96.2%
（2167/2252，85 缺陷类），全集 0 语法错**——数字诚实、无遮蔽。**commons-codec 与 gson 达成完整往返**
...
- **类级干净率第一**：JavaJive **96.2%** > Vineflower **90.8%** > CFR **79.7%**。
- **对 CFR：全面领先。** **8/8 个 jar** 上 JavaJive 缺陷类**均少于** CFR；合计缺陷类 **85 vs 457——比 CFR 少 81%**
  （干净率领先约 16 个百分点）。JSON / 序列化 / 编码族尤为悬殊：gson **0 vs 24**、fastjson2 **15 vs 90**、
  guava **20 vs 154**、commons-codec **0 vs 10**、spring-core **36 vs 117**。
- **对 Vineflower：总量领先、各擅胜场。** 合计缺陷类 **85 vs 208——比 Vineflower 少 59%**，逐 jar 各赢一部分：
  JavaJive 赢 6 个 jar（codec **0 vs 2**、gson **0 vs 16**、jsoup **1 vs 2**、fastjson2 **15 vs 40**、guava **20 vs 66**、
  spring-core **36 vs 74**），snakeyaml 打平（**2 vs 2**，干净率 98.4% 微胜 98.3%），
