...
- **诚实边界**：tree 清零不等于每个方法都已结构化还原（个别方法仍可能走 dump 重建或诚实 stub 后再重建）；
  我们**以 syntax=0 硬断言杜绝“用语法错遮蔽类型错”的虚高**，报告的 100% 是 **tree 口径无遮蔽的诚实值**。
...
| HeapSort | 通过 | 逐字节一致 | 确定性堆排序指纹 |
| KMP | 通过 | 逐字节一致 | 前缀表 + 匹配计数 |
| SwitchFSM | 通过 | 逐字节一致 | fall-through / 嵌套 switch |
| TryFinally | 通过 | 逐字节一致 | 嵌套 try/finally 累加器 |
| UnionFind | 通过 | 逐字节一致 | 路径压缩 + 按秩合并 |
| DiamondTryCatch | 通过 | 逐字节一致 | if-菱形 + 共享 try/catch 循环 |
| DiamondTryFinally | 通过 | 逐字节一致 | tryLock 菱形 + 共享 try/finally |
| ForContinue | 通过 | 逐字节一致 | for + continue 必须跑自增 |
| ComputeIfAbsent | 通过 | 逐字节一致 | get / if-null put 后再用 |
...
| commons-lang3 | 339 | 339/339 | **100.0%** | 0 | 0 |
| jsoup | 148 | 148/148 | **100.0%** | 0 | 0 |
| snakeyaml | 231 | 231/231 | **100.0%** | 0 | 0 |
...
| commons-lang3 | 0 | 0 | 346/346 | ✅ **YES** |
| jsoup | 0 | 0 | 241/241 | ✅ **YES** |
| snakeyaml | 0 | 0 | 233/233 | ✅ **YES** |
