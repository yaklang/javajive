...
本报告主体做 **JavaJive 自评**（与自身历史状态纵向对比进展），并在 **§7 追加与 CFR 0.152 / Vineflower 1.10.1 的同口径三方横向对照**。报告包含两个独立实验：
...
> 应「用事实数据说话」的要求，本节把 JavaJive 放到与两大工业级 Java 反编译器 **CFR**、**Vineflower** 的同口径对照下。
...
`Outer.java`。因此**逐文件率不可跨工具比**，必须归一到**外层（顶层）类**口径。更关键的是**阶段遮蔽**（§3.1.1）：`javac`
...
| commons-lang3 | 345 | 24/198 (87.9%) | 46/198 (76.8%) | **6/198 (97.0%)** |
| jsoup | 238 | **1/51 (98.0%)** | 5/51 (90.2%) | 2/51 (96.1%) |
| snakeyaml | 231 | 4/122 (96.7%) | 11/123 (91.1%) | **2/121 (98.3%)** |
| spring-core | 978 | 90/649 (86.1%) | 117/649 (82.0%) | **74/649 (88.6%)** |
| fastjson2 | 681 | **22/529 (95.8%)** | 90/530 (83.0%) | 41/529 (92.2%) |
| guava | 1892 | 68/558 (87.8%) | 154/558 (72.4%) | **66/558 (88.2%)** |
| **合计** | | **209/2252（90.7%）** | 457/2254（79.7%） | **209/2252（90.7%）** |

> 注：三方外层类总数略有出入（CFR 2254 / 另两方 2252），系各反编译器对合成/匿名类的切分口径不同（gson、fastjson2、
> snakeyaml 各差 1 个）；**干净率百分比**才是公平对齐点。错误行数见 §7.4，仅作上下文（会被少数语法错扭曲，不作口径）。

### 7.3 结论：比 CFR 强多少，比 Vineflower 强多少

- **对 CFR：全面领先。** **8/8 个 jar** 上 JavaJive 缺陷类**均少于** CFR；合计缺陷类 **209 vs 457——比 CFR 少 54%**
  （干净率 **90.7% vs 79.7%，领先 11 个百分点**）。在 JSON / 序列化 / 编码族尤为悬殊：gson **0 vs 24**、
  fastjson2 **22 vs 90**、guava **68 vs 154**、commons-codec **0 vs 10**。
- **对 Vineflower：总量持平、各擅胜场。** 合计缺陷类 **209 vs 209（均 90.7%）**，各赢 4 个 jar。JavaJive 在
  **序列化 / JSON / 编码族更强**：gson **0 vs 16**、fastjson2 **22 vs 41**、commons-codec **0 vs 2**、jsoup **1 vs 2**；
  Vineflower 在**泛型密集族更稳**：commons-lang3 **6 vs 24**、guava 66 vs 68、spring-core 74 vs 90、snakeyaml 2 vs 4。
- **定位**：JavaJive 与 Vineflower 同处当前反编译器**第一梯队**、且**总体正确率并列第一**，全面优于 CFR；而 JavaJive 是三者中
...
| gson | 0 | 103 | 22 |
| commons-lang3 | 78 | 287 | 10 |
| jsoup | 1 | 35 | 3 |
| snakeyaml | 34 | 33 | 2 |
| spring-core | 746 | 764 | 614 |
| fastjson2 | 52 | 623 | 297 |
| guava | 178 | 865 | 132 |

> 行数口径同样**不作主口径**：它既被语法错遮蔽、又随内联/摊平的文件规模波动。此处仅示三方量级——JavaJive 的错误行数在
> JSON/序列化族显著低于二者（fastjson2 52 vs 623 vs 297），在泛型族与 Vineflower 各有高低。

实验环境：darwin/arm64，JDK 21（`javac`/`java` 21，统一 `--release 8`），CFR **0.152**、Vineflower **1.10.1**，
8 jar / 2252 外层类，测于 2026-07-01。
