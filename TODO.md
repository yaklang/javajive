...
> codec 0/0 ✅ · gson 0/0 ✅ · jsoup 1/1 · snakeyaml 1/1 · fastjson2 32/15 · guava 28/20 · commons-lang3 18/11 · spring 57/29。
> (本轮: 三元 class 字面量臂定型(`JDEC_NO_CLASSLIT_SLOT_TYPE`, 与直接存储共用) —— `cond ? Foo.class : classField` 是 `java.lang.Class` 值, 但 class 字面量臂的 `Type()` 报被引类 `Foo`, 致朴素臂合并塌成 `Object`, 局部误声明 `Object c`, `c.getModifiers()/getName()` 失败; 修法把 class 字面量臂计为 `java.lang.Class`(`TernaryArmRValueType`)且声明处优先取槽位 ref 已定型; 治 spring cglib `Enhancer.generateClass`; spring 错误行 63→57; 合计 143→137。)
