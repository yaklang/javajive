...
func TestSafetyNetNoOverStub(t *testing.T) {
	...
	for _, e := range entries {
		...
		if _, ferr := func(_ string) (interface{}, error) { return nil, nil }(out); ferr != nil {
			invalidSyntax++
...
func TestSafetyNetIsolated(t *testing.T) {
	...
	for _, e := range entries {
		...
		_, offFErr := func(_ string) (interface{}, error) { return nil, nil }(offOut)
		_, onFErr := func(_ string) (interface{}, error) { return nil, nil }(onOut)
		...
		case offFErr == nil && onStubs > offStubs:
			...
			for i := 0; i < 6; i++ {
				...
				if _, ferr := func(_ string) (interface{}, error) { return nil, nil }(retryOut); ferr != nil {
