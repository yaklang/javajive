...
func TestParseSignature(t *testing.T) {
	...
	for _, tt := range tests {
		typ := types.ParseSignature(tt.sig)
		if typ == nil {
			t.Errorf("types.ParseSignature(%q) returned nil", tt.sig)
			continue
		}
		got := typ.String(ctx)
		if got != tt.want {
			t.Errorf("types.ParseSignature(%q) = %q, want %q", tt.sig, got, tt.want)
		}
	}
}
...
func TestParseMethodSignature(t *testing.T) {
	...
	params, ret := types.ParseMethodSignature(sig)
	if len(params) != 1 {
