...
import (
	...
	jar "github.com/yaklang/javajive/classparser/decompiler/jar"
)
...
func TestJarParser(t *testing.T) {
	...
	parser, err := jar.NewJarParser(tempJarPath)
	require.NoError(t, err)
...
func TestJarParserFromBytes(t *testing.T) {
	...
	parser, err := jar.NewJarParserFromBytes(jarContent)
	require.NoError(t, err)
	require.NotNil(t, parser)
