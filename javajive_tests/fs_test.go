...
func testJavaJarPositiveDir(t *testing.T) string {
	...
	// Try yaklang's sca testdata path (when running inside yaklang repo)
	dir := filepath.Join(wd, "..", "sca", "testdata", "java_jar", "positive")
	abs, err := filepath.Abs(dir)
	require.NoError(t, err)
	if _, err := os.Stat(abs); err != nil {
		t.Skipf("sca testdata not available: %v", err)
	}
	return abs
}
...
func TestJarFS_RecursiveParse_Enabled(t *testing.T) {
	...
	require.NoError(t, err)
	jarFS := javaclassparser.NewJarFSWithOptions(zipFS, true)
...
func TestJarFS_RecursiveParse_Disabled(t *testing.T) {
	...
	zipFS, err := filesys.NewZipFSFromLocal(jarPath)
	require.NoError(t, err)
	jarFS := javaclassparser.NewJarFSWithOptions(zipFS, false)
...
func TestExpandedLocalFileSystem_ListArchiveAsDirectory(t *testing.T) {
	...
	fs := javaclassparser.NewExpandedLocalFileSystem()
...
func TestJarRecursiveParseEnabled(t *testing.T) {
	require.True(t, javaclassparser.JarRecursiveParseEnabled(nil))
	trueVal := true
	require.True(t, javaclassparser.JarRecursiveParseEnabled(&trueVal))
	falseVal := false
	require.False(t, javaclassparser.JarRecursiveParseEnabled(&falseVal))
}

func TestJarRecursiveParseEnabledFromString(t *testing.T) {
	require.True(t, javaclassparser.JarRecursiveParseEnabledFromString(""))
	require.True(t, javaclassparser.JarRecursiveParseEnabledFromString("true"))
	require.False(t, javaclassparser.JarRecursiveParseEnabledFromString("false"))
	require.True(t, javaclassparser.JarRecursiveParseEnabledFromString("not-a-bool"))
}
...
func TestLocalFileSystemForJarRecursiveParse_Disabled(t *testing.T) {
	...
	fs := javaclassparser.NewLocalFileSystemForJarRecursiveParse(false)
